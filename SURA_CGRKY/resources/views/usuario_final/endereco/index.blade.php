              ...
              @foreach ($usuario->enderecos as $endereco)
                <tr class="hover:bg-gray-800">
                  ...
                  <td class="px-4 py-2 space-x-2">
                    <a href="{{ route('endereco.edit', ['id' => $usuario->id, 'endereco_id' => $endereco->id]) }}" class="text-blue-400 hover:text-blue-600 font-semibold">Editar</a>
                    <form action="{{ route('endereco.destroy', ['id' => $usuario->id, 'endereco_id' => $endereco->id]) }}" method="POST" class="inline">
                      @csrf
                      @method('DELETE')
