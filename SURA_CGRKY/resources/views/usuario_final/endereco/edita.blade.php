...
<div class="fixed top-4 left-4 z-50">
  ...
  <main class="flex flex-col items-center justify-center w-full h-full">
    <section class="bg-black bg-opacity-50 backdrop-blur-md p-8 rounded-3xl shadow-2xl w-full max-w-lg text-white border border-gray-800">
      ...
      <form action="{{ route('endereco.update', ['id' => $usuario->id, 'endereco_id' => $endereco->id]) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <input type="text" name="cidade" value="{{ $endereco->cidade }}" placeholder="Cidade..." required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

        <input type="text" name="cep" value="{{ $endereco->cep }}" placeholder="CEP..." required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

        <input type="text" name="bairro" value="{{ $endereco->bairro }}" placeholder="Bairro..." required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

        <input type="text" name="estado" value="{{ $endereco->estado }}" placeholder="Estado (UF)" maxlength="2" required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

        <input type="text" name="rua" value="{{ $endereco->rua }}" placeholder="Rua..." required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

        <input type="text" name="numero" value="{{ $endereco->numero }}" placeholder="Número..." required
          class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">
