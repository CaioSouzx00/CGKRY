    ...
    <div class="bg-black p-6 rounded-lg shadow-lg w-96">
        ...
        <form method="POST" action="{{ route('password.verificarCodigo') }}">
            ...
            <input type="email" name="email" value="{{ old('email', $email) }}" required readonly
                class="h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm w-full mb-4">
            
            <input type="text" name="token" required placeholder="Digite o código"
                class="h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm w-full mb-4">
            
            <button type="submit" class="btn btn-primary w-full">Verificar Código</button>
        </form>
    </div>
</body>
</html>
