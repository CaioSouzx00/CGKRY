...
<body>
  ...
  @if (session('success'))
  <div class="mb-4 px-4 py-3 rounded-md bg-green-700 text-white text-sm shadow-lg">
    {{ session('success') }}
  </div>
@endif
...
<form method="POST" action="{{ route('fornecedores.store') }}" class="space-y-4">
  ...
  <input type="text" name="cnpj" placeholder="CNPJ" required minlength="14"
    class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

  <input type="text" name="telefone" placeholder="Telefone" required maxlength="15"
    class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

  <input type="email" name="email" placeholder="E-mail" required maxlength="255"
    class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">

  <input type="password" name="senha" placeholder="Senha" required minlength="6" maxlength="255"
    class="w-full h-10 px-4 rounded-md border border-gray-700 bg-gray-800 text-white placeholder-gray-400 text-sm">
  ...
  <button type="submit" class="relative inline-flex items-center justify-start inline-block px-5 py-3 overflow-hidden font-bold rounded-full group">
    ...
    <span class="relative w-full text-left text-white transition-colors duration-200 ease-in-out group-hover:text-gray-900">Cadastrar</span>
